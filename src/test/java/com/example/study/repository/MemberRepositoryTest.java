...
//        teamRepository.save(teamB);
    ...
    void testQueryDSL() {
        // given
        // QueryDSL에서 사용할 QClass 객체를 받아온다.(클래스 내에 상수로 객체가 선언되어 있기 때문에 직접 생성할 필요 없음)
        QMember m = member;

        // when
        Member foundeMember = factory.select(m)
                .from(m)
                .where(m.userName.eq("member1"))
                .fetchOne();

        // then
        assertEquals(foundeMember.getUserName(), "member1");
    }
...
//                .where(member.userName.eq(searchName), member.age.eq(searchAge))
        ...
        /*
         ...
         - fetchOne(): 단일 건 조회. 여러 건 조회 시 예외 발생.
         - fetchFirst(): 단일 건 조회. 여러 개가 조회되도 첫 번째 값만 반환
         - fetch(): List 형태로 반환
...
//                .where(원하는 조건)
    ...
    JPQL : SELECT m, t FROM Member m LEFT JOIN m.team
           ON t.name = 'teamA';
    */
    @Test
